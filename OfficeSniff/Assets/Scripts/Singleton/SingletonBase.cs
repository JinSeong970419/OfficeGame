...
namespace HT
{
    ...
    public class Singleton<T> : SingletonBase<T>, ISingleton where T : class, new()
    {
        ...
        {
            //System.DateTime pCurTime = System.DateTime.Now;

            _isCreateEnabled = false;
            Initialize();

            //System.DateTime pEndTime = System.DateTime.Now;
            //double nMills = (pEndTime - pCurTime).TotalMilliseconds;
            //HTDebug.PrintLog(eMessageType.Good, string.Format("[MANAGER INITIALIZE] {0} init time : {1:F2} sec", typeof(T).ToString(), (nMills * 0.001f)));
        }
